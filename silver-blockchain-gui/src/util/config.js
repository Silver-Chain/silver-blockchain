...
global.daemon_rpc_ws = `wss://${self_hostname}:33810`;
...
function loadConfig(net) {
  try {
    // check if SILVER_ROOT is set. it overrides 'net'
    const config_root_dir =
      'SILVER_ROOT' in process.env
        ? process.env.SILVER_ROOT
        : path.join(os.homedir(), '.silver', net);
    ...
    const daemon_port = lodash.get(config, 'ui.daemon_port', 33810); // jshint ignore:line
