...
const getSilverVersion = () => {
  let version = null;
  const exePath = getExecutablePath('silver');
  // first see if we can get a silver exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or silver is on the path
    try {
      version = child_process
