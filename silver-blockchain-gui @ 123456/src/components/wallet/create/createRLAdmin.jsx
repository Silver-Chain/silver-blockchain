...
import { AlertDialog } from '@silver/core';
import {
  Typography,
  Button,
  Box,
...
import { silver_to_mojo } from '../../../util/silver';
...
export const CreateRLAdminWallet = () => {
  ...
  function create() {
    ...
    if (
      silverper_input.value === '' ||
      Number(silverper_input.value) === 0 ||
      !Number(silverper_input.value) ||
      isNaN(Number(silverper_input.value))
    ) {
      dispatch(
        openDialog(
          <AlertDialog>
    ...
    const silverper = silver_to_mojo(silverper_input.value);
    const silverper_value = Number.parseInt(Number(silverper));
    const userpubkey = userpubkey_input.value;
    const amount = silver_to_mojo(amount_input.value);
    const amount_value = Number.parseInt(Number(amount));
    // var fee = silver_to_mojo(fee_input.value);
