...
from silver.server.outbound_message import NodeType
from silver.server.server import SilverServer, ssl_context_for_client
from silver.server.ws_connection import WSSilverConnection
from silver.ssl.create_ssl import generate_ca_signed_cert
...
async def establish_connection(server: SilverServer, dummy_port: int, ssl_context) -> bool:
...
class TestSSL:
    ...
    async def test_public_connections(self, wallet_node):
        ...
        server_1: SilverServer = full_node_api.full_node.server
    ...
    async def test_farmer(self, harvester_farmer):
        ...
        generate_ca_signed_cert(
            farmer_server.silver_ca_crt_path.read_bytes(), farmer_server.silver_ca_key_path.read_bytes(), pub_crt, pub_key
        )
        ssl_context = ssl_context_for_client(
            farmer_server.silver_ca_crt_path, farmer_server.silver_ca_key_path, pub_crt, pub_key
        )
        connected = await establish_connection(farmer_server, 12312, ssl_context)
    ...
    async def test_full_node(self, wallet_node):
        ...
        generate_ca_signed_cert(
            full_node_server.silver_ca_crt_path.read_bytes(),
            full_node_server.silver_ca_key_path.read_bytes(),
            pub_crt,
            pub_key,
        )
        ssl_context = ssl_context_for_client(
            full_node_server.silver_ca_crt_path, full_node_server.silver_ca_key_path, pub_crt, pub_key
        )
    ...
    async def test_wallet(self, wallet_node):
        ...
        generate_ca_signed_cert(
            wallet_server.silver_ca_crt_path.read_bytes(), wallet_server.silver_ca_key_path.read_bytes(), pub_crt, pub_key
        )
        ssl_context = ssl_context_for_client(
            wallet_server.silver_ca_crt_path, wallet_server.silver_ca_key_path, pub_crt, pub_key
        )
    ...
    async def test_harvester(self, harvester_farmer):
        ...
        generate_ca_signed_cert(
            harvester_server.silver_ca_crt_path.read_bytes(),
            harvester_server.silver_ca_key_path.read_bytes(),
            pub_crt,
            pub_key,
        )
        ssl_context = ssl_context_for_client(
            harvester_server.silver_ca_crt_path, harvester_server.silver_ca_key_path, pub_crt, pub_key
        )
    ...
    async def test_introducer(self, introducer):
        ...
        # Create not authenticated cert
        pub_crt = introducer_server.silver_ca_key_path.parent / "p2p.crt"
        pub_key = introducer_server.silver_ca_key_path.parent / "p2p.key"
        generate_ca_signed_cert(
            introducer_server.silver_ca_crt_path.read_bytes(),
            introducer_server.silver_ca_key_path.read_bytes(),
            pub_crt,
            pub_key,
        )
        ssl_context = ssl_context_for_client(
            introducer_server.silver_ca_crt_path, introducer_server.silver_ca_key_path, pub_crt, pub_key
        )
    ...
    async def test_timelord(self, timelord):
        ...
        generate_ca_signed_cert(
            timelord_server.silver_ca_crt_path.read_bytes(),
            timelord_server.silver_ca_key_path.read_bytes(),
            pub_crt,
            pub_key,
        )
        ssl_context = ssl_context_for_client(
            timelord_server.silver_ca_crt_path, timelord_server.silver_ca_key_path, pub_crt, pub_key
        )
