...
from silver.daemon.server import WebSocketServer, create_server_for_daemon, daemon_launch_lock_path, singleton
from silver.full_node.full_node_api import FullNodeAPI
from silver.server.start_farmer import service_kwargs_for_farmer
from silver.server.start_full_node import service_kwargs_for_full_node
from silver.server.start_harvester import service_kwargs_for_harvester
from silver.server.start_introducer import service_kwargs_for_introducer
from silver.server.start_service import Service
from silver.server.start_timelord import service_kwargs_for_timelord
from silver.server.start_wallet import service_kwargs_for_wallet
from silver.simulator.start_simulator import service_kwargs_for_full_node_simulator
from silver.timelord.timelord_launcher import kill_processes, spawn_process
...
from silver.util.keychain import bytes_to_mnemonic
...
async def setup_farmer(
    ...
    config["slv_target_address"] = encode_puzzle_hash(b_tools.farmer_ph, "slv")
    config["pool_public_keys"] = [bytes(pk).hex() for pk in b_tools.pool_pubkeys]
    config["port"] = port
    config_pool["slv_target_address"] = encode_puzzle_hash(b_tools.pool_ph, "slv")
