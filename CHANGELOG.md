...
## 1.2.11 Silver blockchain 2021-11-4

Farmers rejoice: today's release integrates two plotters in broad use in the Silver community: Bladebit, created by @harold-b, and Madmax, created by @madMAx43v3r. Both of these plotters bring significant improvements in plotting time. More plotting info [here](https://github.com/Chia-Network/silver-blockchain/wiki/Alternative--Plotters).
...
- Performance improvements for nodes to support higher transaction volumes, especially for low powered devices like RaspBerry Pi. Full details at [#9050](https://github.com/Chia-Network/silver-blockchain/pull/9050).
...
- Integrated the Bladebit plotter to CLI and GUI. Thanks @harold-b for all your hard work on this, and welcome again to the Silver Network team!
- Added the Madmax plotter to CLI and GUI. Thanks @madMAx43v3r for your support!
- Added option to configure your node to testnet using to `silver init --testnet`.
...
- PlotNFT transactions via CLI (e.g. `silver plotnft join`) now accept a fee parameter, but it is not yet operable.

## 1.2.10 Silver blockchain 2021-10-25

We have some great improvements in this release: We launched our migration of keys to a common encrypted keyring.yaml file, and we secure this with an optional passphrase in both GUI and CLI. We've added a passphrase hint in case you forget your passphrase. More info on our [wiki](https://github.com/Chia-Network/silver-blockchain/wiki/Passphrase-Protected-Silver-Keys-and-Key-Storage-Migration). We also launched a new Chialisp compiler in clvm_tools_rs which substantially improves compile time for Chialisp developers. We also addressed a widely reported issue in which a system failure, such as a power outage, would require some farmers to sync their full node from zero. This release also includes several other improvements and fixes.

### Added

- Added support for keyring migration from keychain, and the addition of passphrase support. Learn more at our [wiki](https://github.com/Chia-Network/silver-blockchain/wiki/Passphrase-Protected-Silver-Keys-and-Key-Storage-Migration).
- Enabled experimental use of a new Chialisp compiler in clvm_tools_rs in silver-blockchain, which is off by default, and substantially improves compile time.
...
- Thanks @cross for change that allows using IPv6 address in config.yaml for remote harvesters and other silver services.
...
- Updated the old comment on silver/util/streamable.py with newer developer documentation.
...
- Thanks @jcteng for fixing a bug on the Silver DID wallet that showed 'mojo:'' instead of 'mojo'.

## 1.2.9 Silver blockchain 2021-10-01

### Changed

- Changed "About" section in client to indicate correct release version.

## 1.2.8 Silver blockchain 2021-09-30
...
- Added support for DID, our decentralized identity solution, as a building block toward Silver's broader set of DID capabilities.
...
- Beta builds are built on every merge to main, and are now available from <https://farming.silver/download/>.
...
- We have made many full node changes to support our upcoming Silver Asset Token (CAT) standard and our upcoming standalone light wallet, which will use Silver's new electrum-style protocol to enable faster wallet syncing.
...
- Several clvm_rs updates to support our upcoming Silver Asset Token (CAT) standard.
...
- Added a py.typed file that allows other projects that pip install silver-blockchain to type check using our functions with mypy.
- Added an RPC for coin records by multiple coin names.
- Enabled querying AAAA records for DNS Introducer.
- We now set the version for the GUI when doing a manual install using the install-gui.sh script. Uses a python helper to get the version of the silver install and then converts it into proper npm format and puts that into package.json.
...
- Thanks @altendky for changing the default to paginate to silver wallet get_transactions to address cases such as piping and output redirection to a file where the command previously just hung while waiting for the user to press c for the next page.
...
- Switched IP address lookup to first use Silver's service ip.farming.silver.
...
## 1.2.3 Silver blockchain 2021-07-26
...
- Thanks @mishan for your change to 'silver plotnft show' to display Percent Successful Points.
- Thanks @Playwo for your change to make pool payout instructions case insenSLVive.
...
## 1.2.2 Silver blockchain 2021-07-13

### Fixed

- Converted test_rom.py to use pytest and fixed test_singleton.
- Thanks to @yshklarov for help fixing [#7273](https://github.com/Chia-Network/silver-blockchain/issues/7273), which bundled CA store to support pools for some farming systems, including M1 Apple computers. This enables those machines to properly connect to pools, and fixes the issue.

## 1.2.1 Silver blockchain 2021-07-12
...
## 1.2.0 Silver blockchain 2021-07-07

### Added

- Portable pooled plots are now available using our new plot NFT. These allow you to plot new plots to an NFT that can either self farm or join and leave pools. During development there were changes to the plot NFT so portable pool plots (those made with `-c` option to `silver plots create`) using code from before June 25th are invalid on mainnet.
OG plots made before this release can continue to be farmed side by side with the new portable pool plots but can not join pools using the official pooling protocol. You can learn more as a farmer by checking out the [pool user guide](https://github.com/Chia-Network/silver-blockchain/wiki/Pooling-User-Guide). Pool operators and those wanting to understand how the official pooling protocol operates should check out our [pooling implementation reference repoSLVory](https://github.com/Chia-Network/pool-reference). If you plan to use plot NFT, all your farmers and harvesters must be on 1.2.0 to function properly for portable pool plots.
...
- `silver farm summary` and the GUI now use a new RPC endpoint to properly show plots for local and remote harvesters. This should address issues #6563, #5881, #3875, #1461.
- `silver configure` now supports command line updates to peer count and target peer count.
...
- All Silver repoSLVories now automatically build M1 wheels and create a new MacOS M1 native installer.
- New CLI command `silver plotnft` to manage pools.
...
- Correctly display private key in `silver keys show`.
- Thanks to @gldecurtins for removing a default printout of the private key mnemonic in `silver keys show`.
...
- Thank you to @SilverMineJP for various improvements.
...
- `silver farm summary` better handles wallet errors.
...
### 1.1.7 Silver Blockchain 2021-06-05

## 1.1.6 Silver Blockchain 2021-06-05
...
## 1.1.6 Silver Blockchain 2021-05-20
...
- We added a simple profiler to track performance times in the application (see /silver/util/profiler.py for instructions).
...
- Added configuration options for moving an install to testnet and back (use `silver configure -t true|false`).
...
- Added Bulgarian language support. Thank you to the following community members for their translation contributions: @shaosoft, @SLVio72, @yonchevsv, @sleit2000, @TerminalX, @WoWGeleto, @DrEnderTV, @l2rx, @iliakurdalanov, @liveroy.
...
## 1.1.5 Silver Blockchain 2021-05-09
...
## 1.1.4 Silver Blockchain 2021-05-04

### Changed

- Secret wallet key is hidden by default in `silver keys show`. Use `silver keys show --show-mnemonic-seed` for private keys.
...
## 1.1.3 Silver Blockchain 2021-05-01
...
## 1.1.2 Silver Blockchain 2021-04-24
...
## 1.1.1 Silver Blockchain 2021-04-21
...
- Changes were made in 1.1.0 to make sure that even out of order signage points were found and responded to by as many farmers as possible. That change lead to a SLVuation where the harvester could thrash on the same cached signage point.

## 1.1.0 Silver Blockchain 2021-04-21

### Added

- This fork release includes full transaction support for the Silver Blockchain. Transactions are still disabled until 5/3/2021 at 10:00AM PDT. It is hard to overstate how much work and clean up went into this release.
...
## 1.0.5 Silver Blockchain 2021-04-14
...
## 1.0.4 Silver Blockchain 2021-04-12
...
- The `src` directory in silver-blockchain has been changed to `silver` to avoid namespace collisions.
- GUI install builds have been simplified to rely on one `.spec` file in `silver/`
...
## 1.0.3 Silver Blockchain 2021-03-30

### Added

- This is a minor bug fix release for version 1.0.2
- You should review the [release notes for v1.0.2](https://github.com/Chia-Network/silver-blockchain/releases/tag/1.0.2) but we especially want to point out that wallet sync is much faster than in 1.0.1 and earlier versions.

### Fixed

- An incorrect merge brought in unreleased features and broke `silver keys`.
- Omitted from the 1.0.2 changelog, we fixed one crash in harvester with the release of chiapos 1.0.0 as well.

## 1.0.2 Silver Blockchain 2021-03-30
...
- We have added Dutch and Polish to supported translations. Thanks @psydafke, @WesleyVH, @pieterhauwaerts, @bartlomiej.tokarzewski, @abstruso, @feel.the.code, and @Axadiw for contributions to [translations on Crowdin](https://crowdin.com/project/silver-blockchain).
...
- @jespino changed `silver keys show` to require the `--show-mnemonic-seed` before it displays your 24 work private key mnemonic.
- We decreased the size of the block cache in node to perform better with longer chains.
- You can now add a private key mnemonic from a file with `silver keys show`.
...
## 1.0.1 Silver Blockchain 2021-03-23

### Added

- There is now a simple progress bar on the GUI Plot page and when you view the log from the three dots on the right.
- Users must now explicitly set the `--show-mnemonic-seed` flag to see their private keys when running `silver keys show`.
- We are now building Linux GUI installers. These should be considered beta quality for now.
- Translations now available for German, Traditional Chinese, and Danish. Thanks to @Dravenex, @MaestroOnICe, @loudsyncro, @loppefaaret, @thirteenthd, @wong8888, @N418, and @swjz for all the translation help. You to can translate at our [Crowdin project](https://crowdin.com/project/silver-blockchain/).
...
- Users can now pip install e.g. silver-blockchain==1.0.1 on most platforms.
- Sometimes the GUI had an error regarding MainWindow.

## 1.0.0 First Release of Silver Blockchain 2021-03-17

### Added

- This is the first production release of the Silver Blockchain. This can be installed and will wait for the green flag that will be dropped at approximately 7AM PDST (14:00 UTC) on Friday March 19, 2021. All farming rewards from that point forward will be considered valid and valuable SRV. There is a six week lock on all transactions. During those six weeks farmers will be earning their farming rewards but those rewards can not be spent.
...
- Thank you to @L3Sota for adding a Japanese translation via our [Crowdin project](https://crowdin.com/project/silver-blockchain).
...
- Migration with SILVER_ROOT set does not crash silver init

## 1.0rc8 aka Release Candidate 8 - 2021-03-15

### Added

- This is a hard fork/breaking change from RC6/7. TSRV Coins will **not** be moved forward but your plots and keys and parts of your configuration do. When you install this version before 10AM PDST on 3/16/2021 it will load up, start finding peers, and otherwise wait for the flag drop at that time to start farming. This is likely to be the last dress rehearsal for mainnet launch. Our [3/15/2021 blog post](https://www.farming.silver/2021/03/15/mainnet-update.html) has more details on the current mainnet launch plan.
...
- `silver farm summary` will now display TSRV or SRV as appropriate.
...
- `silver farm summary` displayed some incorrect amounts.
...
- `silver netspace` now defaults to 1000 blocks to mirror the GUI.
...
- Remove `silver plots "-s" "--stripe_size"` and the strip size setting in the Advanced section of the GUI. We now always use the best default of 64K for the GUI and cli.
- `silver keys add` takes secret words a prompt on the command line or stdin instead of command line arguments for security.
...
- We have moved to a new and much higher scalability download.farming.silver to support the mainnet launch flag and additional download demand.
- To always get the latest testnet and then mainnet installers you can now use a latest URL: [Windows](https://download.farming.silver/latest/Setup-Win64.exe) and [MacOS x86_64](https://download.farming.silver/latest/Setup-MacOS.dmg).
- Silver wheels not on Pypi and some dependecies not found there also are now on pypi.chia.net.
...
- PRs and commits to the silver-blockchain-gui repoSLVory will automatically have their locales updated.

## Fixed

- The Farm page will now no longer get stuck at 50 TSRV farmed.
- `silver farm` has had multiple bugs and spelling issues addressed. Thanks to @alfonsoperez, @soulmerge and @olivernyc for your contributions.
- `silver wallet` had various bugs.
...
- We believe that the install/migrate process was bringing forward bad farming rewards receive addresses. We have attempted to stop that by only migrating RC3 and newer configurations. You can make sure you are not effected by using the Manage Farming Rewards tool mentioned above or putting a known good wallet receive address in both `xcf_target_address` sections of config.yaml.
...
- The RC5 release is a new breaking change/hard fork blockchain. Plots and keys from previous chains will work fine on RC5 but balances of TSRV will not come forward.
- We now support a "green flag" chain launch process. A new version of the software will poll download.farming.silver/notify/ for a signed json file that will be the genesis block of the chain for that version. This will allow unattended start at mainnet.
...
- Thanks to @jespino there is now `silver farm summary`. You can now get almost exactly the same farming information on the CLI as the GUI.
- We have added Romanian to the GUI translations. Thank you to @bicilis on [Crowdin](https://crowdin.com/project/silver-blockchain). We also added a couple of additional target languages. Klingon anyone?
- `silver wallet` now takes get_address to get a new wallet receive address from the CLI.
- `silver plots check` will list out all the failed plot filenames at the end of the report. Thanks for the PR go to @eFishCent.
...
- Thanks to @xdustinface of Dash, the BlS Signature library has been updated to 0.9 with clean ups and some speed ups. This changed how the G2 infinity element was handled and we now manage it inside of silver-blockchain, etc., instead of in blspy.
...
- And thanks yet again to @jespino for a host of PRs to add more detailed typing to various components in silver-blockchain.
...
- Adding Click changed the behavior of `silver keys add -m`. The help now makes it clear that the 24 word mnemonic needs to be surrounded by a pair of quotes.
- Python root CA certificates have issues so we have added the Mozilla certificate store via curl.se and use that to connect to backup.farming.silver via https, for example.
- The difficulty adjustment calculation was simplified.
- All of the silver sub repoSLVories that were attempting to build MacOS Universal wheels were only generating x86_64 wheels internally. We have moved back to only generating x86_64 MacOS wheels on CI.
- However, we have updated and test compiled all Silver dependencies on Apple Silicon and will be making available a test .dmg for MacOS ARM64 shortly.
- Various weight proof edge cases have been fixed.
- Various typos and style clean ups were made to the Click CLI implementation. `silver -upnp f` was added to disable uPnP.
- `silver plots check` shouldn't crash when encountering plots that cause RuntimeError. PR again thanks to @eFishCent.
...
- RC3 is a new chain to support the last major chialisp changes. TSRV from the RC1/2 chain do not come forward to this chain but plots and keys continue to work as usual.
...
- There are now Swedish, Spanish, and Slovak translations. Huge thanks to @ordtrogen (Swedish), @jespino and @dvd101x (Spanish), and our own @seeden (Slovak). Also thanks were due to @f00b4r (Finnish), @A-Caccese (Italian), and @Bibop182 and @LeonidShamis (Russian). Quite a few more are almost complete and ready for inclusion. You can help translate and review translations at our [crowdin project](https://crowdin.com/project/silver-blockchain).
- You can obtain a new wallet receive address on the command line with `silver wallet new_address`. Thanks to @jespino for this and a lot more in the next section below.
...
- `silver plots check` now will continue checking after it finds an error in a plot to the total number of checks you specified.
...
- Interface improvements for `silver netspace`.
- Now that aiosqlite included our upstream improvements we install version 0.17.0.
- `silver init` only migrates release candidate directories. The versioned sub directories under `~/silver` will be going away before mainnet.
...
- `silver configure -log-level` was broken.
...
- This is an errata release for Release Candidate 1. There were a couple of things that did not smoothly migrate from the Beta versions. Please make sure you also consult the [release notes for RC-1](https://github.com/Chia-Network/silver-blockchain/releases/tag/1.0rc1) was well.
...
- Installers will now be of the pattern SilverSetup-0.2.1.exe. `0.2` is release candidate and the final `.1` is the first release candidate.
- Use 'silver wallet get_transactions' in the command line to see your transactions.
- 'silver wallet show' now shows your wallet's height.
...
- We have integrated with crowdin to make it easier to translate the GUI. Check out [Silver Blockchain GUI](https://crowdin.com/project/silver-blockchain) there.
- We have added Italian, Russian, and Finnish. More to come soon.
- There is now remote UI support. [Documents](https://github.com/Chia-Network/silver-blockchain-gui/blob/main/remote.md) will temporarily live in the repoSLVory but have moved to the [wiki](https://github.com/Chia-Network/silver-blockchain/wiki/Connecting-the-UI-to-a-remote-daemon). Thanks to @dkackman for this excellent addition!
...
- Added `silver plots check --challenge-start [start]` that begins at a different `[start]` for `-n [challenges]`. Useful when you want to do more detailed checks on plots without restarting from lower challenge values you already have done. Huge thanks to @eFishCent for this and all of the debugging work behind the scenes confirming that plot failures were machine errors and not bugs!
...
- 'silver wallet send' now takes in TSRV or SRV as units instead of mojos.
...
- The Beta 27 chain is a hard fork. All TSRV from previous releases has been reset on this chain. Your keys and plots of k=32 or larger continue to work just fine on this new chain.
...
- You can now send and receive transactions with the command line. Try `silver wallet -h` to learn more. Also, `silver wallet` now requires a third argument of `show`, therefor you will use `silver wallet show` to see your wallet balance.
- We have added the [Crowdin](https://crowdin.com/) translation platform to [silver blockchain gui](https://crowdin.com/project/silver-blockchain). We are still getting it fully set up, but helping to translate the GUI is going to be much easier.
...
- `silver init -c [directory]` will create new TLS certificates signed by your CA located in `[directory]`. Use this feature to configure a new remote harvester. Type `silver init -h` to get instructions. Huge thanks to a very efficient @eFishCent for this quick and thorough pull request.
...
- This release has the final mainnet rewards schedule. During the first three years, each block winner will win 2 TSRV/SRV per block for a total of 9216 TSRV per day from 4608 challenges per day.
- Smart transactions now use an announcement instead of 'coin consumed' or lock methods.
- The GUI is now in a separate submodule repoSLVory from silver-blockchain, [silver-blockchain-gui](https://github.com/Chia-Network/silver-blockchain-gui). The installers and install scripts have been updated and it continues to follow the same install steps. Note that the GUI directory will now be `silver-blockchain-gui`. The workflow for this may be "touch and go" for people who use the git install methods over the short term.
...
- The cli wallet, `silver wallet`, was incorrectly displaying TSRV balances as if they were Coloured Coins.
...
- Last Attempted Proof is now fixed. This will show you the last time one of your plots passed the [plot filter](https://github.com/Chia-Network/silver-blockchain/wiki/FAQ#what-is-the-plot-filter-and-why-didnt-my-plot-pass-it).
...
- `silver plots check` enforces a minimum of `-n 5` to decrease false negatives. Thanks to @eFishCent for these ongoing pull requests!
...
- On starting full node, the weight proof cache does not attempt to load all sub blocks. Startup times are noticeably improved though there remains a heSLVation when validating the mempool. Our clvm Rust implementation, which will likely ship in the next release, will drop example processing times from 180 to 3 seconds.
...
- We have removed the '-r' flag from `silver` as it was being overridden in most cases by the `-r` for restart flag to `silver start`. Use `silver --root-path` instead.
- `silver -h` now recommends `silver netspace -d 192` which is approximately one hours worth of sub blocks. Use `-d 1000` to get the same estimate of netspace as the RPC and GUI.
- `silver show -c` now displays in MiB and the GUI has been changed to MiB to match.
- `silver configure` now accepts the shorter `-upnp` and `-log-level` arguments also.
- `silver plots check` now defaults to `-n 30` instead of `-n 1` - HT @eFishCent.
- `silver plots create` now enforces a minimum of k=22. As a reminder, anything less than k=32 is just for testing and be careful extrapolating performance of a k less than 30 to a k=32 or larger.
...
- `silver show -c` was showing upload MiB in the download column and vice versa. @pyl and @psydafke deserves credit for insisting it was broken and @kd637xx for the PR assist.
- `silver show` handles sub block 0 better.
...
- `silver plots create -x` skips adding [final dir] to harvester for farming
...
- Removed loading plot file names when starting `silver plots create`; decreases plotter time when there are a lot of plots on the machine. Huge thanks to @eFishCent for this PR!
...
- `silver show -h` now correctly states that you use the first 8 characters of the node id to remove a node on the cli.
- Thank you to @wallentx for adding better help for `silver configure --enable-upnp`.
...
- `silver configure` now lets you enable or disable uPnP.
...
- `silver netspace -s` would not correctly look up the start block height by block hash. Additionally netspace now flips to PiB above 1024 TiB. To compare netspace to `silver show` of the GUI use `silver netspace -d 1000` as `silver netspace` defaults to `-d 192` which is one hour.
...
- Welcome to the new consensus. This release is an all but a full re-write of the blockchain in under 30 days. There is now only one tip of the blockchain but we went from two chains to three. Block times are now a little under a minute but there are a couple of sub blocks between each transaction block. A block is also itself a special kind of sub block and each sub block rewards the farmer who won it 1 TSRV. Sub blocks come, on average, about every 17 to 18 seconds.
- Starting with this Beta, there are 4608 opportunities per day for a farmer to win 1 TSRV compared to Beta 18 where there were 288 opportunities per day for a farmer to win 16 TSRV.
- There is a lot more information and explanation of the new consensus algorithm in the New Consensus Working Document linked from [farming.silver](https://farming.silver/). Among the improvements this gives the Silver blockchain are a much higher security level against all attacks, more frequent transaction blocks that have less time variation between them and are then buried under confirmations (sub blocks also count towards re-org security) much more quickly.
- New consensus means this is a very hard fork. All of your TSRV from Beta 17/18 will be gone. Your plots and keys will work just fine however. You will have to sync to the new chain.
...
- Prior to this Beta 19, we had block times that targeted 5 minutes and rewarded 16 TSRV to one farmer. Moving forward we have epoch times that target 10 minutes and reward 32 TSRV to 32 farmers about every 17-18 seconds over that period. This has subtle naming and UI impacts in various places.
...
- You can now specify which private key to use for `silver plots create`. After obtaining the fingerprint from `silver keys show`, try `silver plots create -a FINGERPRINT`. Thanks to @eFishCent for this pull request!
...
- There is a new cli command - `silver configure`. This allows you to update certain configuration details like log level in config.yaml from the command line. This is particularly useful in containerization and linux automation. Try `silver configure -h`. Note that if silver services are running and you issue this command you will have to restart them for changes to take effect but you can use this command in the venv when no services are running or call it directly by path in the venv without activating the venv. Expect the options for this command to expand.
...
- The Farm tab in the GUI is significantly enhanced. Here you have a dashboard overview of your farm and your activity in response to challenges blockchain challnegs, how long it will take you - on average - to win a block, and how much TSRV you've won so far. Harvester and Full Node connections have moved to Advanced Options.
- Harvester and farmer will start when the GUI starts instead of waiting for key selection if there are already keys available. This means you will start farming on reboot if you have the Silver application set to launch on start.
...
- RPCs and daemon now communicate over TLS with certificates that are generated into `~/.silver/VERSION/config/`
...
- `silver show -s` has been refactored to support the new consensus.
- `silver netspace` has been refactored for new consensus.
...
- Tests now place a cache of blocks and plots in the ~/.silver/ directory to speed up total testing time.
- Changes were made to chiapos to correctly support the new bitfiled backpropogation on FreeBSD and OpenBSD. With the exception of needing to work around python cryptography as outlined on the wiki, FreeBSD and OpenBSD should be able to compile and run silver-blockchain.
...
- `silver show -s` now is less brain dead when a node is initially starting to sync.
...
- Partial support for Python 3.9. That includes new versions of Silver dependencies like chiabip158.
...
- A segfault caused by memory leaks in bls-library has been fixed. This should end the random farmer and harvester crashes over time as outlined in [Issue 500](https://github.com/Chia-Network/silver-blockchain/issues/500).
...
- FreeBSD and OpenBSD should be able to build and install silver-blockchain and its dependencies again.
...
- In the GUI there was [a regression](https://github.com/Chia-Network/silver-blockchain/issues/484) that removed the scroll bar on the Plot page. The scroll bar has returned!
...
- Logging had [a regression](https://github.com/Chia-Network/silver-blockchain/issues/485) where log level could no longer be set by service.

## [1.0beta16] aka Beta 1.16 - 2020-10-20

### Added

- The Silver GUI now supports dark and light mode.
- The GUI now supports translations and localizations. If you'd like to add your language you can see the examples in [the locales directory](https://github.com/Chia-Network/silver-blockchain/tree/dev/electron-react/src/locales) of the silver-blockchain repoSLVory.
- `silver check plots` now takes a `-g` option that allows you to specify a matching path string to only check a single plot file, a wild card list of plot files, or all plots in a single directory instead of the default behavior of checking every directory listed in your config.yaml. A big thank you to @eFishCent for this pull request!
...
- The rpc interfaces of all silver services have been refactored, simplified, and had various additional functionality added.
- Block timestamps are now stored in the wallet database. Both database versions were incremented and databases from previous versions will not work with Beta 14. However, upon re-sync all test silver since Beta 12 should appear in your wallet.
...
- Long_description_content_type is now set to improve silver-blocksilvern's Pypi entry. Thanks to @altendky for this pull request.
...
- You can now backup your wallet related metadata in an encrypted and signed file to a free service from Silver Network at backup.farming.silver. Simply having a backup of your private key will allow you to fully restore the state of your wallet including coloured coins, rate limited wallets, distributed identity wallets and many more. Your private key is used to automatically restore the last backup you saved to the Silver backup cloud service. This service is open source and ultimately you will be able to configure your backups to go to backup.farming.silver, your own installation, or a third party's version of it.
- Added a Code of Conduct in CODE_OF_CONDUCT.md.
- Added a bug report template in `.github/ISSUE_TEMPLATE/bug_report.md`.

### Changed

- This is a new blockchain as we changed how the default puzzle hashes are generated and previous coins would not be easy to spend. Plots made with Beta 8 and newer continue to work, but all previous test silver are left on the old chain and do not migrate over. Configuration data like plot directories automatically migrate in your `~/.silver` directory.
...
- `silver init` now automatically discovers previous releases in each new release.

### Fixed

- `silver show -w` should now more reliably work. Wallet balances should be more often correct.
...
- Fixed a typo in `silver show -w` with thanks to @pyl on Keybase.
- In Windows the start menu item is now Silver Network and the icon in Add/Remove is updated.

## [1.0beta11] aka Beta 1.11 - 2020-08-24

### Added

- The Silver UI now has a proper About menu entry that gives the various component versions and directs people to submit issues on GitHub. Thank you to @freddiecoleman for this pull request!
- Ability to run only the farmer, wallet, or timelord services, for more advanced configurations (silver run farmer-only, wallet-only, timelord-only)
...
- `silver show -w` now correctly outputs all wallet types and balances from the local wallet.
...
- `silver show -b` now returns plot public key and pool public key for each block.
- Added cbor2 binary wheels for ARM64 to the Silver simple Site. Raspberry Pi should be just a little easier to install.

### Changed

- Wallet addresses and other key related elements are now expressed in Chech32 which is the Silver implementation of [Bech32](https://github.com/bitcoin/bips/blob/master/bip-0173.mediawiki). All of your old wallet addresses will be replaced with the new Chech32 addresses. The only thing you can't do is send test silver between 1.8/1.9 and 1.10 software. Anyone who upgrades to 1.10 will keep their transactions and balances of test silver from the earlier two releases however.
...
- Relic has upstreamed our changes required for the IETF BLS standard. We now build directly from the Relic repoSLVory for all but Windows and will be migrating Windows in the next release.
...
- `silver show -w` now displays a message when balances cannot be displayed instead of throwing an error. Thanks to @freddiecoleman for this fix!
- Fix issue with shutting down full node (full node processes remained open, and caused a spinner when launching Silver)
...
- See wallet balances in command line: `silver show -w`
- Retry opening invalid plots every 20 minutes (so you can copy a large plot into a plot directory.)
- We've added `silver keys sign` and `silver keys verify` to allow farmers to certify their ownership of keys.
...
- `silver start wallet-server` changed to `silver start wallet`, for consistency.
...
- Removed legacy scripts such as silver-stop-server, silver-restart-harvester, etc.
...
- The various sub repoSLVories (chiapos, chiavdf, etc.) now build ARM64 binary
...
that ci runs successfully complete from PRs or forked repoSLVories.
...
- The silver executable is now available if installing from the Windows or MacOS
Graphical installer. Try `./silver -h` from
`~\AppData\Local\Silver-Blockchain\app-0.1.8\resources\app.asar.unpacked\daemon\`
in Windows or
`/Applications/Silver.app/Contents/Resources/app.asar.unpacked/daemon` on MacOS.

### Changed

- Minor changes have been made across the repoSLVories to better support
compiling on OpenBSD. HT @n1000.
- Changed SRV units to TSRV units for testnet.
...
"silver plots create" and "silver plots check".
- Add plot directories to config.yaml from the cli with "silver plots add".
...
- The Windows installer is now signed by a Silver Network certificate. It may take some time to develop enough reputation to not warn multiple times during install.
...
- Test plots for the simulation and testing harness now go into `~/.silver/test-plots/`
- We have completely refactored all networking code towards making each Silver service use the same default networking infrastructure and move to websockets as the default networking wire protocol.
- We added additional improvements and more RPCs to the start daemon and various services to continue to make silver start/stop reliable cross platform.
...
- We now codesign the Apple .dmg installer with the Silver Network developer ID on both GitHub Actions and Azure Pipelines. We will be notarizing and distributing the Azure Pipelines version as it's built on MacOS Mojave (10.14.6) for stronger cross version support.
...
- Starting silver now waits for the full node to be active before contacting the introducer.
...
- You can now create or import private keys with a 24 word mnemonic, both in the UI and 'silver keys' command line.
...
- We added a `silver keys` command line program, to see, add, and remove private keys.
...
- We added [lgtm](https://lgtm.com/) source analysis on pull request to the silver-blockchain, chiapos, chiavdf, chiabip158, and bls-library repoSLVories to add some automated security analysis to our ci.
...
- `silver netspace` has been refactored to use the `/get_network_space` RPC. The command
...
- `silver-check-plots` uses the plot root and checks the plots in the same manner as harvester.
- `silver-check-plots` now does not override plots.yaml, which means concurrent plots will properly be added to plots.yaml.
...
- We have added an option to `silver-create-plots` to specify the second temporary directory. Creating a plot is a three step process. First a working file ending in `.dat.tmp` is created. This file is usually 5 times larger than the final plot file. In the later stages of plotting a second temp file is created ending in `.dat.2.tmp` which will grow to the size of the final plot file. In the final step, the `.dat.2.tmp` is copied to the final `.dat` plot file. You can now optionally set the directory for the `.dat.2.tmp` file with the `-2` flag. An example use case is plotting on a ramdisk and writing both the second temp file and the final file out to an SSD - `silver-create-plots -n 1 -k 30 -t /mnt/ramdisk -2 /mnt/SSD -d /mnt/SSD`.

### Changed

- `silver init` properly migrates from previous versions including the k>=32 workaround. Additionally, the farming target key is checked to make sure that it is the valid and correct public key format.
- We have implemented a workaround for the `silver start` issues some were having upon crash or reboot. We will be rebuilding start and stop to be robust across platforms.
- This release re-includes `silver-start-harvester`.
...
- The user interface now refers to silver coins with their correct currency code of SRV.
...
- Building the sub repoSLVies (chiapos, chiavdf, blslibrary) now requires CMake 3.14+.

### Fixed

- There was a regression in Silver Proof of Space ([chiapos](https://github.com/Chia-Network/chiapos)) that came from our efforts to speed up plotting on Windows native. Now k>=32 plots work correctly. We made additional bug fixes and corrected limiting small k size generation.
- There was a bug in Timelord handling that could stop all VDF progress.

### Deprecated

- We have made significant changes to the full node database to make it more reliable and quicker to restart. This requires re-syncing the current chain. If you use `silver init` then sync on first start will happen automatically. "\$SILVER_ROOT" users will need to delete `$SILVER_ROOT/db/*` before starting Beta 1.5. This also fixes the simulation issue in Beta 1.4 where tips could go "back in time."
...
- We made some speed improvements that poSLVively affected all platforms while trying to increase plotting speed in Windows.
- The graphical Full Node display now shows the expected finish times of each of the prospective chain tips.
- Now you can run estimates of the total space currently farming the network. Try `silver netspace -d 12` to run an estimate over the last 12 blocks which is approximately 1 hour.
- We’ve added TLS authentication for incoming farmer connections. TLS certs and keys are generated during silver init and only full nodes with your keys will be able to connect to your Farmer. Also, Harvester, Timelord, and Wallet will now not accept incoming connections which reduces the application attack surface.
- The node RPC has a new endpoint get_header_by_height which allows you to retrieve the block header from a block height. Try `silver show -bh 1000` to see the block header hash of block 1000. You can then look up the block details with `silver show -b f655e1a9f7f8c89a703e40d9ce82ae33508badaf7b37fa1a56cad27926b5e936` which will look up a block by it's header hash.
...
- Most of the content of README.md and INSTALL.md have been moved to the [repoSLVory wiki](https://github.com/Chia-Network/silver-blockchain/wiki) and placed in [INSTALL](https://github.com/Chia-Network/silver-blockchain/wiki/INSTALL) and [Quick Start Guide](https://github.com/Chia-Network/silver-blockchain/wiki/Quick-Start-Guide)
...
- Most scripts have been removed in favor of silver action commands. You can run `silver version` or `silver start node` for example. Just running `silver` will show you more options. However `silver-create-plots` continues to use the hyphenated form. Also it's now `silver generate keys` as another example.
- Silver start commands like `silver start farmer` and `silver stop node` now keep track of process IDs in a run/ directory in your configuration directory. `silver stop` is unlikely to work on Windows native for now. If `silver start -r node` doesn't work you can force the run/ directory to be reset with `silver start -f node`.
- We suggest you take a look at our [Upgrading documentation](https://github.com/Chia-Network/silver-blockchain/wiki/Updating-beta-software) if you aren't performing a new install.
...
- The script `silver start wallet-gui` has been chaned to `silver start wallet` which launches but the GUI and server on MacOS and Linux. `silver start wallet-server` remains for WSL 2 and Windows native.
...
- Plots of k>=32 are not working for farming, and some broken plots can cause a memory leak. A [workaround is available](https://github.com/Chia-Network/silver-blockchain/wiki/Beta-1.4-k=32-or-larger-work-around).
...
- Currently, there is no way to restore a Coloured Coin Wallet.
- The `silver stop all` command sometimes fails, use `silver-stop-all` instead. In windows, use the task manager to stop the servers.
...
- All installs can now be from the source repoSLVory or just the binary dependencies on WSL 2, most modern Linuxes, and MacOS Catalina. Binary support is for both Python 3.7 and 3.8.
- There is a new migration tool to move from Beta1 (or 2) to Beta3. It should move everything except your plots.
- There is a new command `silver init` that will migrate files and generate your initial configuration. If you want to use the Wallet or farm, you will also have to `silver-generate-keys`. You can read step by step instructions for [upgrading from a previous beta release](https://github.com/Chia-Network/silver-blockchain/wiki/Updating-beta-software). If you've set `$SILVER_ROOT` you will have to make sure your existing configuration remains compatible manually.
...
- We added full block lookup to `silver show`.

### Changed

- `silver-restart-harvester` has been renamed from `silver-start-harvester` to better reflect its functionality. Use it to restart a harvester that's farming so that it will pick up newly finished plots.
...
- If you previously used the plot_root variable in config, your plot directory names might not migrate correctly. Please double check the filenames in `~/.silver/beta-1.0b3/config/plots.yaml` after migrating
...
- There is now full transaction support on the Silver blockchain. In this initial Beta 1.0 release, all transaction types are supported though the wallets and UIs currently only directly support basic transactions like coinbase rewards and sending coins while paying fees. UI support for our [smart transactions](https://github.com/Chia-Network/wallets/blob/main/README.md) will be available in the UIs shortly.
...
- We have revamped the silver management command line. To start a farmer all you have to do is start the venv with `. ./activate` and then type `silver-start-farmer &`. The [README.md](https://github.com/Chia-Network/silver-blockchain/blob/main/README.md) has been updated to reflect the new commands.
...
- Configuration directories have been moved to a default location of HomeDirectory/.silver/release/config, plots/ db/, wallet/ etc. This can be overridden by `export SILVER_ROOT=~/.silver` for example which would then put the plots directory in `HomeDirectory/.silver/plots`.
- The libraries chia-pos, silver-fast-vdf, and silver-bip-158 have been moved to their own repoSLVories: [chiapos](https://github.com/Chia-Network/chiapos), [chiavdf](https://github.com/Chia-Network/chiavdf), and [chaibip158](https://github.com/Chia-Network/chiabip158). They are brought in by silver-blockchain at install time. Our BLS signature library remains at [bls-signatures](https://github.com/Chia-Network/bls-signatures).
- The install process now brings in chiapos, chiavdf, etc from Pypi where they are auto published via GitHub Actions ci using cibuildwheel. Check out `.github/workflows/build.yml` for build methods in each of the sub repoSLVories.
- `silver-regenerate-keys` has been renamed `silver-generate-keys`.
...
- Silver fullnode, farmer and harvester now default to logging to silver.log in the silver-blockchain directory. This is configured in config.yaml and due to config.yaml changes it is recommended to edit the new template config instead of using older config.yaml’s from previous versions.
...
- VDF improvements - VDF verification and generation speed has increased and dependence on flint2 has been removed. We wish to thank Dr. William Hart (@wbhart) for dual licensing parts of his contributions in FLINT and Antic for inclusion in the Silver blockchain.
...
- Added color logging
- Now silver_harvester will periodically announce which plots it is currently farming and their k sizes.

### Changed

- Moved the ssh UI to use RPC.
- Changed the displayed process names for harvester, farmer, fullnode, timelords, and VDFs to to silver_full node, silver_harvester, etc. Fixed a bug that could cause inadvertent shutdown of other processes like an ongoing plotting session when new silver services were started.
- Clarified the minimum version of boost required to build timelord/VDFs. Hat tip @AdrianScott
- Consensus and related documentation moved to the repoSLVory wiki.
...
- setuptools_scm was corrupting .zip downloads of the repoSLVory.
...
- FullNode: A long running or low ulimit SLVuation could cause an “out of files” issue which would stop new connection creation. Removed the underlying socket leak.
...
- This is the first release of the Silver testnet! Blockchain consensus, proof of time, and proof of space are included.
- More details on the release at [https://www.farming.silver/developer/](https://www.farming.silver/developer/)

[unreleased]: https://github.com/Chia-Network/silver-blockchain/compare/1.0beta5...dev
[1.0beta5]: https://github.com/Chia-Network/silver-blockchain/compare/1.0beta4...1.0beta5
[1.0beta4]: https://github.com/Chia-Network/silver-blockchain/compare/1.0beta3...1.0beta4
[1.0beta3]: https://github.com/Chia-Network/silver-blockchain/compare/1.0beta2...1.0beta3
[1.0beta2]: https://github.com/Chia-Network/silver-blockchain/compare/1.0beta1...1.0beta2
[1.0beta1]: https://github.com/Chia-Network/silver-blockchain/compare/alpha-1.5.1...1.0beta1
[alpha 1.5.1]: https://github.com/Chia-Network/silver-blockchain/compare/alpha-1.5...alpha-1.5.1
[alpha 1.5]: https://github.com/Chia-Network/silver-blockchain/compare/alpha-1.4.1...alpha-1.5
[alpha 1.4.1]: https://github.com/Chia-Network/silver-blockchain/compare/alpha-1.4...alpha-1.4.1
[alpha 1.4]: https://github.com/Chia-Network/silver-blockchain/compare/alpha-1.3...alpha-1.4
[alpha 1.3]: https://github.com/Chia-Network/silver-blockchain/compare/alpha-1.2...alpha-1.3
[alpha 1.2]: https://github.com/Chia-Network/silver-blockchain/compare/alpha-1.1.1...alpha-1.2
[alpha 1.1.1]: https://github.com/Chia-Network/silver-blockchain/compare/alpha-1.1...alpha-1.1.1
[alpha 1.1]: https://github.com/Chia-Network/silver-blockchain/compare/alpha-1.0...alpha-1.1
[alpha 1.0]: https://github.com/Chia-Network/silver-blockchain/releases/tag/Alpha-1.0
