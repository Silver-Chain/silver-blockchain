...
import { mojo_to_silver } from '../../util/silver';
import {
  calculatePoolReward,
...
export default function Block() {
  ...
  const poolReward = mojo_to_silver(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojo_to_silver(
    calculateBaseFarmerReward(blockRecord.height),
  );

  const silverFees = blockRecord.fees
    ? mojo_to_silver(BigInt(blockRecord.fees))
    : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://alltheblocks.net/silver/puzzlehash/{blockRecord.farmer_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://alltheblocks.net/silver/puzzlehash/{blockRecord.pool_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: silverFees ? `${silverFees} ${currencyCode}` : '',
      tooltip: (
        <Trans>
    ...
    <LayoutMain title={<Trans>Block</Trans>}>
      <Card
        title={
          <Back variant="h5">
            <Trans>
              Block at height {blockRecord.height} in the Silver blockchain
            </Trans>
          </Back>
        }
        action={
