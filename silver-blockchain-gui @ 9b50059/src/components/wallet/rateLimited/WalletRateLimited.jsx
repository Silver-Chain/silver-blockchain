...
import { AlertDialog, Card, Flex } from '@silver/core';
import {
  send_transaction,
...
const IncompleteCard = (props) => {
  ...
  function submit() {
    ...
    const silverper_input = ip_parsed.limit;
    ...
    const silverper_value = Number.parseInt(Number(silverper_input));
        ...
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  Spending Limit (silver per interval):{' '}
                  {mojo_to_silver_string(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>
...
const SendCard = (props) => {
  ...
  function send() {
    ...
    const fee = silver_to_mojo(fee_input.value);
