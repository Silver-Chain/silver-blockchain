...
import silverEnvironment from '../util/silverEnvironment';
import silverConfig from '../util/config';
...
if (!handleSquirrelEvent()) {
  ...
  const ensureCorrectEnvironment = () => {
    ...
    if (!silverEnvironment.guessPackaged() && !('VIRTUAL_ENV' in process.env)) {
  ...
  if (ensureSingleInstance() && ensureCorrectEnvironment()) {
    ...
    silverConfig.loadConfig('mainnet');
    ...
    const createWindow = async () => {
      ...
      if (!silverConfig.manageDaemonLifetime()) {
      ...
      mainWindow.on('close', (e) => {
        ...
        if (decidedToClose || !silverConfig.manageDaemonLifetime()) {
          return;
        }
        e.preventDefault();
        if (!isClosing) {
    ...
    const appReady = async () => {
      ...
      // if the daemon isn't local we aren't going to try to start/stop it
      if (silverConfig.manageDaemonLifetime()) {
        silverEnvironment.startSilverDaemon();
      }
    };
  ...
  const getMenuTemplate = () => {
    const template = [
      ...
      },
      {
        ...
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'Silver Blockchain GitHub' }),
            click: () => {
              openExternal(
                'https://github.com/silver-network/silver-blockchain/',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://www.farming.silver',
              );
            },
          },
          {
            label: i18n._(/* i18n */ { id: 'Silver Release Notes' }),
            click: () => {
              openExternal(
                'https://github.com/silver-network/silver-blockchain/releases',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/silver-network/silver-blockchain/blob/master/CONTRIBUTING.md',
              );
            },
          },
          {
          ...
          // },
          {
            ...
            click: () => {
              openExternal(
                'https://chiaforkscalculator.com/silver',
              );
            },
          },
          // {
          ...
          // },
          {
            label: i18n._(/* i18n */ { id: 'Silver on AllTheBlocks' }),
            click: () => {
              openExternal(
                'https://alltheblocks.net/silver',
              );
            },
          },
          {
          ...
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/silver-network/silver-blockchain/issues',
              );
            },
          },
          // {
          ...
          //   click: () => {
          //     openExternal('https://twitter.com/SilverProject');
          //   },
          // },
        ],
      },
    ...
    if (process.platform === 'darwin') {
      // Silver Blockchain menu (Mac)
      template.unshift({
        label: i18n._(/* i18n */ { id: 'Silver' }),
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'About Silver Blockchain' }),
            click: () => {
              openAbout();
            },
          },
