...
from silver.types.peer_info import PeerInfo, TimestampedPeerInfo
from silver.util.hash import std_hash
from silver.util.ints import uint16, uint64
...
class ExtendedPeerInfo:
    ...
    def get_bucket_poSLVion(self, key: int, is_new: bool, nBucket: int) -> int:
...
class AddressManager:
    ...
    used_new_matrix_poSLVions: Set[Tuple[int, int]]
    used_tried_matrix_poSLVions: Set[Tuple[int, int]]
    ...
    def clear(self) -> None:
        ...
        self.used_new_matrix_poSLVions = set()
        self.used_tried_matrix_poSLVions = set()
    ...
    def _set_new_matrix(self, row: int, col: int, value: int) -> None:
        ...
        if value == -1:
            if (row, col) in self.used_new_matrix_poSLVions:
                self.used_new_matrix_poSLVions.remove((row, col))
        else:
            if (row, col) not in self.used_new_matrix_poSLVions:
                self.used_new_matrix_poSLVions.add((row, col))
    ...
    def _set_tried_matrix(self, row: int, col: int, value: int) -> None:
        ...
        if value == -1:
            if (row, col) in self.used_tried_matrix_poSLVions:
                self.used_tried_matrix_poSLVions.remove((row, col))
        else:
            if (row, col) not in self.used_tried_matrix_poSLVions:
                self.used_tried_matrix_poSLVions.add((row, col))

    def load_used_table_poSLVions(self) -> None:
        self.used_new_matrix_poSLVions = set()
        self.used_tried_matrix_poSLVions = set()
        for bucket in range(NEW_BUCKET_COUNT):
            for pos in range(BUCKET_SIZE):
                if self.new_matrix[bucket][pos] != -1:
                    self.used_new_matrix_poSLVions.add((bucket, pos))
        for bucket in range(TRIED_BUCKET_COUNT):
            for pos in range(BUCKET_SIZE):
                if self.tried_matrix[bucket][pos] != -1:
                    self.used_tried_matrix_poSLVions.add((bucket, pos))
    ...
    def make_tried_(self, info: ExtendedPeerInfo, node_id: int) -> None:
        for bucket in range(NEW_BUCKET_COUNT):
            pos = info.get_bucket_poSLVion(self.key, True, bucket)
        ...
        cur_bucket_pos = info.get_bucket_poSLVion(self.key, False, cur_bucket)
        if self.tried_matrix[cur_bucket][cur_bucket_pos] != -1:
            ...
            self.tried_count -= 1
            # Find its poSLVion into new table.
            new_bucket = old_info.get_new_bucket(self.key)
            new_bucket_pos = old_info.get_bucket_poSLVion(self.key, True, new_bucket)
    ...
    def mark_good_(self, addr: PeerInfo, test_before_evict: bool, timestamp: int) -> None:
        ...
        for n in range(NEW_BUCKET_COUNT):
            ...
            cur_new_bucket_pos = info.get_bucket_poSLVion(self.key, True, cur_new_bucket)
        ...
        tried_bucket_pos = info.get_bucket_poSLVion(self.key, False, tried_bucket)
    ...
    def add_to_new_table_(self, addr: TimestampedPeerInfo, source: Optional[PeerInfo], penalty: int) -> bool:
        ...
        new_bucket_pos = info.get_bucket_poSLVion(self.key, True, new_bucket)
    ...
    def select_peer_(self, new_only: bool) -> Optional[ExtendedPeerInfo]:
        ...
        if not new_only and self.tried_count > 0 and (self.new_count == 0 or randrange(2) == 0):
            chance = 1.0
            start = time.time()
            cached_tried_matrix_poSLVions: List[Tuple[int, int]] = []
            if len(self.used_tried_matrix_poSLVions) < math.sqrt(TRIED_BUCKET_COUNT * BUCKET_SIZE):
                cached_tried_matrix_poSLVions = list(self.used_tried_matrix_poSLVions)
            while True:
                if len(self.used_tried_matrix_poSLVions) < math.sqrt(TRIED_BUCKET_COUNT * BUCKET_SIZE):
                    if len(self.used_tried_matrix_poSLVions) == 0:
                        log.error(f"Empty tried table, but tried_count shows {self.tried_count}.")
                        return None
                    # The table is sparse, randomly pick from poSLVions list.
                    index = randrange(len(cached_tried_matrix_poSLVions))
                    tried_bucket, tried_bucket_pos = cached_tried_matrix_poSLVions[index]
                else:
                    # The table is dense, randomly trying poSLVions is faster than loading poSLVions list.
            ...
            chance = 1.0
            start = time.time()
            cached_new_matrix_poSLVions: List[Tuple[int, int]] = []
            if len(self.used_new_matrix_poSLVions) < math.sqrt(NEW_BUCKET_COUNT * BUCKET_SIZE):
                cached_new_matrix_poSLVions = list(self.used_new_matrix_poSLVions)
            while True:
                if len(self.used_new_matrix_poSLVions) < math.sqrt(NEW_BUCKET_COUNT * BUCKET_SIZE):
                    if len(self.used_new_matrix_poSLVions) == 0:
                        log.error(f"Empty new table, but new_count shows {self.new_count}.")
                        return None
                    index = randrange(len(cached_new_matrix_poSLVions))
                    new_bucket, new_bucket_pos = cached_new_matrix_poSLVions[index]
                else:
    ...
    def select_tried_collision_(self) -> Optional[ExtendedPeerInfo]:
        ...
        tried_bucket_pos = new_info.get_bucket_poSLVion(self.key, False, tried_bucket)
