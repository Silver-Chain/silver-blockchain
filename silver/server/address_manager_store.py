...
from silver.server.address_manager import (
    BUCKET_SIZE,
    NEW_BUCKET_COUNT,
...
class AddressManagerStore:
    ...
    * Once we know the buckets, we can also deduce the bucket poSLVions.
    ...
    async def deserialize(self) -> AddressManager:
        ...
        for node_id, info in tried_table_nodes:
            ...
            tried_bucket_pos = info.get_bucket_poSLVion(address_manager.key, False, tried_bucket)
        ...
        for node_id, bucket in new_table_entries:
            if node_id >= 0 and node_id < address_manager.new_count:
                ...
                bucket_pos = info.get_bucket_poSLVion(address_manager.key, True, bucket)
        ...
        address_manager.load_used_table_poSLVions()
