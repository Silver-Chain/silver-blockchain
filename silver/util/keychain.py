...
DEFAULT_USER = f"user-silver-{CURRENT_KEY_VERSION}"  # e.g. user-silver-1.8
DEFAULT_SERVICE = f"silver-{DEFAULT_USER}"  # e.g. silver-user-silver-1.8
...
def passphrase_requirements() -> Dict[str, Any]:
    ...
    return {"is_optional": True, "min_length": MIN_PASSPHRASE_LEN}  # lgtm [py/clear-text-logging-senSLVive-data]
...
def obtain_current_passphrase(prompt: str = DEFAULT_PASSPHRASE_PROMPT, use_passphrase_cache: bool = False) -> str:
    ...
    before failing.
    """
    from silver.cmds.passphrase_funcs import prompt_for_passphrase
...
def bytes_to_mnemonic(mnemonic_bytes: bytes) -> str:
    ...
    for i in range(0, len(bitarray) // 11):
        ...
        m_word_poSLVion = bits.uint
        m_word = word_list[m_word_poSLVion]
