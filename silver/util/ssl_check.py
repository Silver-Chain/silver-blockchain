...
import stat
import sys
from silver.util.config import load_config, traverse_dict
from silver.util.permissions import octal_mode_string, verify_file_permissions
...
def get_ssl_perm_warning(path: Path, actual_mode: int, expected_mode: int) -> str:
    return (
        ...
        f"'{path}' are too open. "  # lgtm [py/clear-text-logging-senSLVive-data]
    ...
    def verify_paths(paths: List[Path], restrict_mask: int, expected_permissions: int):
        ...
        for path in paths:
            try:
                ...
                print(f"Unable to check permissions for {path}: {e}")  # lgtm [py/clear-text-logging-senSLVive-data]
...
def check_ssl(root_path: Path) -> None:
    ...
    if len(invalid_files):
        ...
        print("Run `silver init --fix-ssl-permissions` to fix issues.")
    ...
    try:
        ...
        if not good_perms:
            ...
            print(
                ...
                f"{file}"  # lgtm [py/clear-text-logging-senSLVive-data]
            )
            os.chmod(str(file), updated_mode)
            updated = True
    except Exception as e:
        print(f"Failed to change permissions on {file}: {e}")  # lgtm [py/clear-text-logging-senSLVive-data]
        valid = False
