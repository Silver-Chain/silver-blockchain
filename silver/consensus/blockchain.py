...
from silver.consensus.block_body_validation import validate_block_body
from silver.consensus.block_header_validation import validate_finished_header_block, validate_unfinished_header_block
...
from silver.consensus.coinbase import create_puzzlehash_for_pk
...
from silver.consensus.multiprocess_validation import (
    PreValidationResult,
    _run_generator,
    pre_validate_blocks_multiprocessing,
)
from silver.consensus.pos_quality import UI_ACTUAL_SPACE_CONSTANT_FACTOR
...
from silver.full_node.mempool_check_conditions import get_name_puzzle_conditions
...
from silver.types.blockchain_format.vdf import VDFInfo
from silver.types.coin_record import CoinRecord
from silver.types.condition_opcodes import ConditionOpcode
from silver.types.end_of_slot_bundle import EndOfSubSlotBundle
from silver.types.full_block import FullBlock
from silver.types.generator_types import BlockGenerator, GeneratorArg
...
from silver.types.weight_proof import SubEpochChallengeSegment
from silver.util.errors import ConsensusError, Err
from silver.util.generator_tools import get_block_header, tx_removals_and_additions
from silver.util.ints import uint16, uint32, uint64, uint128
from silver.util.streamable import recurse_jsonify
...
class Blockchain(BlockchainInterface):
    ...
    async def validate_unfinished_block(
        ...
        if error_code is not None:
            return PreValidationResult(uint16(error_code.value), None, None, None)

        return PreValidationResult(None, required_iters, cost_result, str(difficulty_coeff))
    ...
    async def get_farmer_difficulty_coeff(
        ...
        coeff = Decimal(0)
        space = 0
        if minimal_staking == 0 or staking < minimal_staking:
            coeff = Decimal(20)
        else:
            if blocks == 0:
                coeff = Decimal(1)
            else:
                # multiple 1000 to keep same scale as SLV
    ...
    async def get_peak_farmer_staking(self, farmer_public_key: G1Element, peak: Optional[BlockRecord]) -> uint64:
        ph = create_puzzlehash_for_pk(farmer_public_key)
        coins = await self.coin_store.get_unspent_coins_before_height(ph, peak.height if peak is not None else 0)
