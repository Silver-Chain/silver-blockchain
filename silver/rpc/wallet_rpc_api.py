...
from silver.consensus.block_rewards import calculate_base_farmer_reward
from silver.consensus.coinbase import create_puzzlehash_for_pk
from silver.pools.pool_puzzles import SINGLETON_MOD_HASH, create_p2_singleton_puzzle
from silver.pools.pool_wallet import PoolWallet
from silver.pools.pool_wallet_info import FARMING_TO_POOL, PoolState, PoolWalletInfo, create_pool_state
from silver.protocols.protocol_message_types import ProtocolMessageTypes
from silver.server.outbound_message import NodeType, make_msg
from silver.simulator.simulator_protocol import FarmNewBlockProtocol
from silver.types.blockchain_format.coin import Coin
from silver.types.blockchain_format.program import Program, SerializedProgram
from silver.types.blockchain_format.sized_bytes import bytes32
from silver.util.bech32m import decode_puzzle_hash, encode_puzzle_hash
from silver.util.byte_types import hexstr_to_bytes
from silver.util.config import load_config
from silver.util.ints import uint32, uint64
from silver.util.keychain import KeyringIsLocked, bytes_to_mnemonic, generate_mnemonic
from silver.util.path import path_from_root
from silver.util.ws_message import WsRpcMessage, create_payload_dict
...
from silver.wallet.trade_record import TradeRecord
from silver.wallet.transaction_record import TransactionRecord
from silver.wallet.util.backup_utils import download_backup, get_backup_info, upload_backup
from silver.wallet.util.trade_utils import trade_record_to_dict
from silver.wallet.util.transaction_type import TransactionType
from silver.wallet.util.wallet_types import WalletType
from silver.wallet.wallet_info import WalletInfo
from silver.wallet.wallet_node import WalletNode
...
class WalletRpcApi:
    def __init__(self, wallet_node: WalletNode):
        ...
        self.service_name = "silver_wallet"
    ...
    async def _state_changed(self, *args) -> List[WsRpcMessage]:
        ...
        return [create_payload_dict("state_changed", data, "silver_wallet", "wallet_ui")]
    ...
    async def _check_key_used_for_rewards(
        ...
        farmer_target = config["farmer"].get("xcf_target_address")
        pool_target = config["pool"].get("xcf_target_address")
        found_farmer = False
        found_pool = False
    ...
    async def create_new_wallet(self, request: Dict):
        ...
        if request["wallet_type"] == "cc_wallet":
            ...
            if request["mode"] == "new":
                ...
                from silver.pools.pool_wallet_info import initial_pool_state_from_dict
    ...
    async def pw_self_pool(self, request) -> Dict:
        if self.service.wallet_state_manager is None:
            return {"success": False, "error": "not_initialized"}
        # Leaving a pool requires two state tranSLVions.
        # First we tranSLVion to PoolSingletonState.LEAVING_POOL
        # Then we tranSLVion to FARMING_TO_POOL or SELF_POOLING
        fee = uint64(request.get("fee", 0))
        wallet_id = uint32(request["wallet_id"])
        wallet: PoolWallet = self.service.wallet_state_manager.wallets[wallet_id]
