...
from silver.plotters.bladebit import get_bladebit_install_info, plot_bladebit
from silver.plotters.chiapos import get_chiapos_install_info, plot_silver
from silver.plotters.madmax import get_madmax_install_info, plot_madmax
from silver.plotters.install_plotter import install_plotter
...
def call_plotters(root_path: Path, args):
    # Add `plotters` section in SILVER_ROOT.
    silver_root_path = root_path
    ...
    if not os.path.exists(root_path):
        print(f"Creating plotters folder within SILVER_ROOT: {root_path}")
        try:
            os.mkdir(root_path)
        except Exception as e:
    ...
    if args.plotter == "chiapos":
        plot_silver(args, silver_root_path)
    if args.plotter == "madmax":
        plot_madmax(args, silver_root_path, root_path)
    if args.plotter == "bladebit":
        plot_bladebit(args, silver_root_path, root_path)
