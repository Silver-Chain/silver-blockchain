...
class WalletPoolStore:
    ...
    _state_tranSLVions_cache: Dict[int, List[Tuple[uint32, CoinSpend]]]
    ...
    async def create(cls, wrapper: DBWrapper):
        ...
        await self.db_connection.execute(
            "CREATE TABLE IF NOT EXISTS pool_state_tranSLVions(tranSLVion_index integer, wallet_id integer, "
            "height bigint, coin_spend blob, PRIMARY KEY(tranSLVion_index, wallet_id))"
        )
    ...
    async def add_spend(
        ...
        get_all_state_tranSLVions.
        """
        if wallet_id not in self._state_tranSLVions_cache:
            self._state_tranSLVions_cache[wallet_id] = []
        all_state_tranSLVions: List[Tuple[uint32, CoinSpend]] = self.get_spends_for_wallet(wallet_id)

        if (height, spend) in all_state_tranSLVions:
            return

        if len(all_state_tranSLVions) > 0:
            if height < all_state_tranSLVions[-1][0]:
                raise ValueError("Height cannot go down")
            if spend.coin.parent_coin_info != all_state_tranSLVions[-1][1].coin.name():
                raise ValueError("New spend does not extend")

        all_state_tranSLVions.append((height, spend))

        cursor = await self.db_connection.execute(
            "INSERT OR REPLACE INTO pool_state_tranSLVions VALUES (?, ?, ?, ?)",
            (
                len(all_state_tranSLVions) - 1,
                wallet_id,
                height,
                bytes(spend),
            ),
    ...
    def get_spends_for_wallet(self, wallet_id: int) -> List[Tuple[uint32, CoinSpend]]:
        ...
        """
        return self._state_tranSLVions_cache.get(wallet_id, [])
    ...
    async def rebuild_cache(self) -> None:
        ...
        are removed. This can happen if a state tranSLVion in wallet_blockchain fails.
        """
        cursor = await self.db_connection.execute("SELECT * FROM pool_state_tranSLVions ORDER BY tranSLVion_index")
        rows = await cursor.fetchall()
        await cursor.close()
        self._state_tranSLVions_cache = {}
        for row in rows:
            _, wallet_id, height, coin_spend_bytes = row
            coin_spend: CoinSpend = CoinSpend.from_bytes(coin_spend_bytes)
            if wallet_id not in self._state_tranSLVions_cache:
                self._state_tranSLVions_cache[wallet_id] = []
            self._state_tranSLVions_cache[wallet_id].append((height, coin_spend))
    ...
    async def rollback(self, height: int, wallet_id_arg: int) -> None:
        ...
        get_all_state_tranSLVions.
        """
        for wallet_id, items in self._state_tranSLVions_cache.items():
        ...
        cursor = await self.db_connection.execute(
            "DELETE FROM pool_state_tranSLVions WHERE height>? AND wallet_id=?", (height, wallet_id_arg)
        )
        await cursor.close()
