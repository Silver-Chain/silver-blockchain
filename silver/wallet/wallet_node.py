...
from silver.daemon.keychain_proxy import (
    KeychainProxy,
...
from silver.pools.pool_puzzles import SINGLETON_LAUNCHER_HASH
from silver.protocols import farmer_protocol, wallet_protocol
from silver.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
...
from silver.util.keychain import Keychain
from silver.util.lru_cache import LRUCache
from silver.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
...
from silver.wallet.settings.settings_objects import BackupInitialized
...
from silver.wallet.wallet_blockchain import ReceiveBlockResult
from silver.wallet.wallet_state_manager import WalletStateManager
...
class WalletNode:
    ...
    async def get_key_for_fingerprint(self, fingerprint: Optional[int]) -> Optional[PrivateKey]:
        ...
        try:
            ...
            self.log.warning("No keys present. Create keys with the UI, or with the 'silver keys' program.")
            return None
    ...
    async def on_connect(self, peer: WSSilverConnection):
        if self.wallet_state_manager is None or self.backup_initialized is False:
            return None
    ...
    async def update_stakings(self, peer: WSSilverConnection, height: uint64, farmer_public_key: G1Element) -> None:
        "fetch staking"
    ...
    async def complete_blocks(self, header_blocks: List[HeaderBlock], peer: WSSilverConnection):
    ...
    async def new_peak_wallet(self, peak: wallet_protocol.NewPeakWallet, peer: WSSilverConnection):
        if self.wallet_state_manager is None:
            return
    ...
    async def _sync(self) -> None:
        ...
        peers: List[WSSilverConnection] = self.server.get_full_node_connections()
        if len(peers) == 0:
    ...
    async def get_additions(
        self, peer: WSSilverConnection, block_i, additions: Optional[List[bytes32]], get_all_additions: bool = False
    ) -> Optional[List[Coin]]:
    ...
    async def get_removals(
        self, peer: WSSilverConnection, block_i, additions, removals, request_all_removals=False
    ) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
...
async def wallet_next_block_check(
    peer: WSSilverConnection, potential_peek: uint32, blockchain: BlockchainInterface
) -> bool:
