...
from silver.pools.pool_config import PoolWalletConfig, load_pool_config, update_pool_config
from silver.pools.pool_wallet_info import (
    PoolWalletInfo,
    PoolSingletonState,
    PoolState,
    FARMING_TO_POOL,
...
from silver.protocols.pool_protocol import POOL_PROTOCOL_VERSION
...
from silver.pools.pool_puzzles import (
...
from silver.util.ints import uint8, uint32, uint64
...
class PoolWallet:
    ...
    def get_next_interesting_coin_ids(spend: CoinSpend) -> List[bytes32]:
        ...
        # If we return a value, it is a coin ID that we are also interested in (to support two tranSLVions per block)
        coin: Optional[Coin] = get_most_recent_singleton_coin_from_coin_spend(spend)
        if coin is not None:
            return [coin.name()]
        return []

    async def apply_state_tranSLVions(self, block_spends: List[CoinSpend], block_height: uint32):
        """
    ...
    async def create_new_pool_wallet_transaction(
        ...
        the same pooling puzzle. This puzzle is a `silver singleton` that is
