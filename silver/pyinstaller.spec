...
ROOT = pathlib.Path(importlib.import_module("silver").__file__).absolute().parent.parent


def solve_name_collision_problem(analysis):
    """
    There is a collision between the `silver` file name (which is the executable)
    and the `silver` directory, which contains non-code resources like `english.txt`.
    We move all the resources in the zipped area so there is no
    need to create the `silver` directory, since the names collide.
    ...
    for data in analysis.datas:
        if str(data[0]).startswith("silver/"):
            zipped.append(data)
        else:
            datas.append(data)
...
version_data = copy_metadata(get_distribution("silver-blockchain"))[0]
...
# TODO: collapse all these entry points into one `silver_exec` entrypoint that accepts the server as a parameter

entry_points = ["silver.cmds.silver"] + [f"silver.server.start_{s}" for s in SERVERS]
...
if THIS_IS_WINDOWS:
    entry_points.extend(["aiohttp", "silver.util.bip39"])

if THIS_IS_WINDOWS:
    silver_mod = importlib.import_module("silver")
    dll_paths = ROOT / "*.dll"
...
datas.append((f"{ROOT}/silver/util/english.txt", "silver/util"))
datas.append((f"{ROOT}/silver/util/initial-config.yaml", "silver/util"))
datas.append((f"{ROOT}/silver/wallet/puzzles/*.hex", "silver/wallet/puzzles"))
datas.append((f"{ROOT}/silver/ssl/*", "silver/ssl"))
...
add_binary("silver", f"{ROOT}/silver/cmds/silver.py", COLLECT_ARGS)
add_binary("daemon", f"{ROOT}/silver/daemon/server.py", COLLECT_ARGS)

for server in SERVERS:
    add_binary(f"start_{server}", f"{ROOT}/silver/server/start_{server}.py", COLLECT_ARGS)
