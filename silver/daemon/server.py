...
from silver.cmds.init_funcs import check_keys, silver_init
from silver.cmds.passphrase_funcs import default_passphrase, using_default_passphrase
from silver.daemon.keychain_server import KeychainServer, keychain_commands
from silver.daemon.windows_signal import kill
from silver.plotters.plotters import get_available_plotters
from silver.plotting.util import add_plot_directory
from silver.server.server import ssl_context_for_root, ssl_context_for_server
from silver.ssl.create_ssl import get_mozilla_ca_crt
from silver.util.silver_logging import initialize_logging
from silver.util.config import load_config
from silver.util.json_util import dict_to_json_str
from silver.util.keychain import (
    Keychain,
...
from silver.util.service_groups import validate_service
from silver.util.setproctitle import setproctitle
from silver.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Silver.")
    quit()
...
service_plotter = "silver plots create"
...
if getattr(sys, "frozen", False):
    name_map = {
        "silver": "silver",
        "silver_wallet": "start_wallet",
        "silver_full_node": "start_full_node",
        "silver_harvester": "start_harvester",
        "silver_farmer": "start_farmer",
        "silver_introducer": "start_introducer",
        "silver_timelord": "start_timelord",
        "silver_timelord_launcher": "timelord_launcher",
        "silver_full_node_simulator": "start_simulator",
    }
...
class WebSocketServer:
    ...
    def _build_plotting_command_args(self, request: Any, ignoreCount: bool, index: int) -> List[str]:
        ...
        command_args: List[str] = ["silver", "plotters", plotter]
    ...
    def _post_process_plotting_job(self, job: Dict[str, Any]):
        ...
        log.info(f"Post-processing plotter job with ID {id}")  # lgtm [py/clear-text-logging-senSLVive-data]
    ...
    async def _start_plotting(self, id: str, loop: asyncio.AbstractEventLoop, queue: str = "default"):
        ...
        try:
            log.info(f"Starting plotting with ID {id}")  # lgtm [py/clear-text-logging-senSLVive-data]
            ...
            log.exception(f"problem starting {service_name}")  # lgtm [py/clear-text-logging-senSLVive-data]
    ...
    async def start_plotting(self, request: Dict[str, Any]):
        ...
        for k in range(count):
            ...
            ids.append(id)
            config = {
                "id": id,  # lgtm [py/clear-text-logging-senSLVive-data]
                "size": size,
                "queue": queue,
                "plotter": plotter,
                "service_name": service_name,
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("silver daemon exiting in 5 seconds")
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered SILVER_ROOT
    os.environ["SILVER_ROOT"] = str(root_path)
    ...
    log.info(f"Service array: {service_array}")  # lgtm [py/clear-text-logging-senSLVive-data]
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with SILVER_ROOT: {os.environ['SILVER_ROOT']}")
    ...
    if service_command == "silver_full_node_simulator":
        # Set the -D/--connect_to_daemon flag to signify that the child should connect
        # to the daemon to access the keychain
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    # When wait_for_unlock is true, we want to skip the check_keys() call in silver_init
    # since it might be necessary to wait for the GUI to unlock the keyring first.
    silver_init(root_path, should_check_keys=(not wait_for_unlock))
    config = load_config(root_path, "config.yaml")
    setproctitle("silver_daemon")
...
def main(argv) -> int:
    from silver.util.default_root import DEFAULT_ROOT_PATH
