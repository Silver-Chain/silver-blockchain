...
# from silver.util.ssl_check import DEFAULT_PERMISSIONS_CERT_FILE, DEFAULT_PERMISSIONS_KEY_FILE
...
def get_silver_ca_crt_key() -> Tuple[Any, Any]:
    crt = pkg_resources.resource_string(__name__, "silver_ca.crt")
    key = pkg_resources.resource_string(__name__, "silver_ca.key")
    return crt, key
...
def write_ssl_cert_and_key(cert_path: Path, cert_data: bytes, key_path: Path, key_data: bytes):
    ...
    for path, data, mode in [
        ...
        with open(os.open(str(path), flags, mode), "wb") as f:
            f.write(data)  # lgtm [py/clear-text-storage-senSLVive-data]
...
def generate_ca_signed_cert(ca_crt: bytes, ca_key: bytes, cert_out: Path, key_out: Path):
    ...
    new_subject = x509.Name(
        [
            x509.NameAttribute(NameOID.COMMON_NAME, "Silver"),
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "Silver Network"),
    ...
    cert = (
        ...
        .add_extension(
            x509.SubjectAlternativeName([x509.DNSName("farming.silver")]),
            critical=False,
        )
...
def make_ca_cert(cert_path: Path, key_path: Path):
    ...
    subject = issuer = x509.Name(
        [
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "Silver"),
            x509.NameAttribute(NameOID.COMMON_NAME, "Silver CA"),
            x509.NameAttribute(NameOID.ORGANIZATIONAL_UNIT_NAME, "Organic Farming Division"),
        ]
    )
    root_cert = (
...
def main():
    return make_ca_cert(Path("./silver_ca.crt"), Path("./silver_ca.key"))
