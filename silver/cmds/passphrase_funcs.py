...
from silver.daemon.client import acquire_connection_to_daemon
from silver.util.keychain import Keychain, obtain_current_passphrase, supports_os_passphrase_storage
from silver.util.keyring_wrapper import DEFAULT_PASSPHRASE_IF_NO_MASTER_PASSPHRASE
from silver.util.misc import prompt_yes_no
from silver.util.ws_message import WsRpcMessage
    ...
    try:
        if supports_os_passphrase_storage():
            ...
            print(
                ...
                "Other Silver processes will be able to access your keys without prompting for your passphrase."
            )
...
def prompt_for_new_passphrase() -> Tuple[str, bool]:
    ...
    if min_length > 0:
        n = min_length
        print(f"\nPassphrases must be {n} or more characters in length")  # lgtm [py/clear-text-logging-senSLVive-data]
    while True:
        ...
        if valid_passphrase:
            ...
            print(f"{error_msg}\n")  # lgtm [py/clear-text-logging-senSLVive-data]
...
def initialize_passphrase() -> None:
    if Keychain.has_master_passphrase():
        ...
        print("\nUse 'silver passphrase set' or 'silver passphrase remove' to update or remove your passphrase")
        sys.exit(1)
...
def display_passphrase_hint() -> None:
    ...
    if passphrase_hint is not None:
        print(f"Passphrase hint: {passphrase_hint}")  # lgtm [py/clear-text-logging-senSLVive-data]
    else:
