...
from silver.cmds.configure import configure_cmd
from silver.cmds.farm import farm_cmd
from silver.cmds.init import init_cmd
from silver.cmds.keys import keys_cmd
from silver.cmds.netspace import netspace_cmd
from silver.cmds.passphrase import passphrase_cmd
from silver.cmds.plotnft import plotnft_cmd
from silver.cmds.plots import plots_cmd
from silver.cmds.plotters import plotters_cmd
from silver.cmds.show import show_cmd
from silver.cmds.start import start_cmd
from silver.cmds.stop import stop_cmd
from silver.cmds.wallet import wallet_cmd
from silver.util.default_root import DEFAULT_KEYS_ROOT_PATH, DEFAULT_ROOT_PATH
from silver.util.keychain import set_keys_root_path, supports_keyring_passphrase
from silver.util.ssl_check import check_ssl
...
@click.group(
    help=f"\n  Manage silver blockchain infrastructure ({__version__})\n",
    epilog="Try 'silver start node', 'silver netspace -d 192', or 'silver show -s'",
...
def cli(
    ...
    if passphrase_file is not None:
        from silver.cmds.passphrase_funcs import cache_passphrase, read_passphrase_from_file
...
if not supports_keyring_passphrase():
    from silver.cmds.passphrase_funcs import remove_passphrase_options_from_cmd

    # TODO: Remove once keyring passphrase management is rolled out to all platforms
    remove_passphrase_options_from_cmd(cli)


@cli.command("version", short_help="Show silver version")
def version_cmd() -> None:
    print(__version__)


@cli.command("run_daemon", short_help="Runs silver daemon")
@click.option(
    "--wait-for-unlock",
    help="If the keyring is passphrase-protected, the daemon will wait for an unlock command before accessing keys",
    default=False,
    is_flag=True,
    hidden=True,  # --wait-for-unlock is only set when launched by silver start <service>
)
@click.pass_context
def run_daemon_cmd(ctx: click.Context, wait_for_unlock: bool) -> None:
    import asyncio

    from silver.daemon.server import async_run_daemon
