...
from silver.server.start_wallet import SERVICE_NAME
...
def print_transaction(tx: TransactionRecord, verbose: bool, name) -> None:
    if verbose:
        ...
        silver_amount = Decimal(int(tx.amount)) / units["silver"]
        to_address = encode_puzzle_hash(tx.to_puzzle_hash, name)
        print(f"Transaction {tx.name}")
        print(f"Status: {'Confirmed' if tx.confirmed else ('In mempool' if tx.is_in_mempool() else 'Pending')}")
        print(f"Amount {'sent' if tx.sent else 'received'}: {silver_amount} {name}")
...
async def send(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    final_amount = uint64(int(amount * units["silver"]))
    final_fee = uint64(int(fee * units["silver"]))
    ...
    while time.time() - start < 10:
        ...
        if len(tx.sent_to) > 0:
            ...
            print(f"Do silver wallet get_transaction -f {fingerprint} -tx 0x{tx_id} to get status")
            return None
...
def wallet_coin_unit(typ: WalletType, address_prefix: str) -> Tuple[str, int]:
    ...
    if typ in [WalletType.STANDARD_WALLET, WalletType.POOLING_WALLET, WalletType.MULTI_SIG, WalletType.RATE_LIMITED]:
        return address_prefix, units["silver"]
    return "", units["mojo"]
...
async def send_from(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    withdraw_amount = uint64(int(request_amount * units["silver"]))
    coin_records = None
...
async def get_wallet(wallet_client: WalletRpcClient, fingerprint: int = None) -> Optional[Tuple[WalletRpcClient, int]]:
    ...
    if len(fingerprints) == 0:
        print("No keys loaded. Run 'silver keys generate' or import a key")
        return None
