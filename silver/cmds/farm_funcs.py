...
from silver.cmds.units import units
...
from silver.rpc.full_node_rpc_client import FullNodeRpcClient
...
from silver.util.misc import format_bytes, format_minutes
from silver.util.network import is_localhost
...
async def summary(
    ...
    if amounts is not None:
        print(f"Total silver farmed: {amounts['farmed_amount'] / units['silver']}")
        print(f"User transaction fees: {amounts['fee_amount'] / units['silver']}")
        print(f"Block rewards: {(amounts['farmer_reward_amount'] + amounts['pool_reward_amount']) / units['silver']}")
    ...
    if amounts is None:
        if wallet_not_running:
            print(
                "For details on farmed rewards and fees you should run 'silver start wallet' and 'silver wallet show'"
            )
        elif wallet_not_ready:
            print("For details on farmed rewards and fees you should run 'silver wallet show'")
    else:
        print("Note: log into your key using 'silver wallet show' to see rewards for each key")
