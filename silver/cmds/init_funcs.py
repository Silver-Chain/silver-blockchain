...
from silver.ssl.create_ssl import (
    ensure_ssl_dirs,
    generate_ca_signed_cert,
    get_silver_ca_crt_key,
    make_ca_cert,
    write_ssl_cert_and_key,
)
from silver.util.bech32m import encode_puzzle_hash
from silver.util.config import (
    create_default_silver_config,
    initial_config_file,
    load_config,
    save_config,
    unflatten_properties,
)
from silver.util.ints import uint32
from silver.util.keychain import Keychain
from silver.util.path import mkdir
from silver.util.ssl_check import (
...
from silver.wallet.derive_keys import master_sk_to_pool_sk, master_sk_to_wallet_sk
from silver.cmds.configure import configure
...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    if len(all_sks) == 0:
        print("No keys are present in the keychain. Generate them with 'silver keys generate'")
        return None
...
def create_all_ssl(root_path: Path):
    ...
    silver_ca_crt, silver_ca_key = get_silver_ca_crt_key()
    silver_ca_crt_path = ca_dir / "silver_ca.crt"
    silver_ca_key_path = ca_dir / "silver_ca.key"
    write_ssl_cert_and_key(silver_ca_crt_path, silver_ca_crt, silver_ca_key_path, silver_ca_key)
    ...
    silver_ca_crt, silver_ca_key = get_silver_ca_crt_key()
    generate_ssl_for_nodes(ssl_dir, silver_ca_crt, silver_ca_key, False, overwrite=False)
...
def init(create_certs: Optional[Path], root_path: Path, fix_ssl_permissions: bool = False, testnet: bool = False):
    if create_certs is not None:
        if root_path.exists():
            ...
            if (
                silver_init(root_path, fix_ssl_permissions=fix_ssl_permissions, testnet=testnet) == 0
        ...
        return silver_init(root_path, fix_ssl_permissions=fix_ssl_permissions, testnet=testnet)


def silver_version_number() -> Tuple[str, str, str, str]:
...
def silver_minor_release_number():
    res = int(silver_version_number()[2])
    print(f"Install release number: {res}")
    return res


def silver_full_version_str() -> str:
    major, minor, patch, dev = silver_version_number()
...
def silver_init(
    ...
    """
    if os.environ.get("SILVER_ROOT", None) is not None:
        print(
            f"warning, your SILVER_ROOT is set to {os.environ['SILVER_ROOT']}. "
            f"Please unset the environment variable and run silver init again\n"
            f"or manually migrate config.yaml"
        )

    print(f"Silver directory {root_path}")
    if root_path.is_dir() and Path(root_path / "config" / "config.yaml").exists():
        # This is reached if SILVER_ROOT is set, or if user has run silver init twice
        # before a new update.
        if testnet:
    ...
    if should_check_keys:
        check_keys(root_path)
    print("")
    print("To see your keys, run 'silver keys show --show-mnemonic-seed'")
